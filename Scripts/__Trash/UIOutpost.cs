...
/*
public partial class UIOutpost : Control
{
    ...
    public void Select(UIBuildingsItem building)
    {
        ...
        for (int idx = 0; idx < Main.Count; idx++)
        {
            if (Main[idx] == building)
            {
                ...
                else
                {
                    ...
                    {
                        //if (ActionBuildDistrict.GetAvailableBuilding(Game, _Sector, upgradeLinks[upgradeIdx].ValueS, _Planet) != null)
                        {
                            upgrades.Add(Game.Def.GetDistrict(upgradeLinks[upgradeIdx].ValueS));
                        }
                    }
        ...
        for (int idx = 0; idx < Upgrades.Count; idx++)
        {
            ...
            {
                //DefDistrictWrapper action = ActionBuildDistrict.GetAvailableBuilding(Game, _Sector, building._BuildingDef.ValueS, _Planet);
                //if (action != null)
                //{
                //    //ActionBuildDistrict.AddToQueue(Game, action);
